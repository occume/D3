...
//		testFileDistribution(conn);
	...
	public void testAddUpdateContact(Jedis conn) {
		...
		for (int i = 0; i < 10; i++) {
			addUpdateContact(conn, "user",
					"contact-" + ((int) Math.floor(i / 3)) + '-' + i);
		}
	...
	public void testAddressBookAutocomplete(Jedis conn) {
		...
		System.out.println();
		System.out
				.println("now let's try to find users with names starting with 'jen':");
		Set<String> r = autocompleteOnPrefix1(conn, "test", "jen");
		System.out.println(r);
		assert r.size() == 3;

		System.out.println("jeff just left to join a different guild...");
		leaveGuild(conn, "test", "je");
		r = autocompleteOnPrefix1(conn, "test", "asdf");
		System.out.println(r);
		assert r.size() == 2;
//		conn.del("members:test");
	}
	...
	public void testDistributedLocking(Jedis conn) throws InterruptedException {
		...
		System.out
				.println("Trying to get it again without releasing the first one...");
	...
	public void testDelayedTasks(Jedis conn) throws InterruptedException {
		...
		for (long delay : new long[] { 0, 500, 0, 1500 }) {
			assert executeLater(conn, "tqueue", "testfn",
					new ArrayList<String>(), delay) != null;
		}
		long r = conn.llen("queue:tqueue");
		System.out
				.println("How many non-delayed tasks are there (should be 2)? "
						+ r);
		assert r == 2;
		System.out.println();

		System.out
				.println("Let's start up a thread to bring those delayed tasks back...");
		...
		System.out
				.println("Waiting is over, how many tasks do we have (should be 4)? "
						+ r);
		assert r == 4;
		conn.del("queue:tqueue", "delayed:");
	}
	...
	public void testMultiRecipientMessaging(Jedis conn) {
		...
		conn.del("ids:chat:", "msgs:1", "ids:1", "seen:joe", "seen:jeff",
				"seen:jenny");

		System.out
				.println("Let's create a new chat session with some recipients...");
		...
		System.out
				.println("And let's get the messages that are waiting for jeff and jenny...");
	...
	public void testFileDistribution(Jedis conn) throws InterruptedException,
			IOException {
		...
		conn.del("msgs:test:", "seen:0", "seen:source", "ids:test:",
				"chat:test:");
		...
		System.out
				.println("Let's pause to let some logs get copied to Redis...");
		Thread.sleep(250);
		System.out.println();
		System.out
				.println("Okay, the logs should be ready. Let's process them!");

		System.out.println("Files should have 1, 100, and 1000 lines");
		TestCallback callback = new TestCallback();
		processLogsFromRedis(conn, "0", callback);
		System.out.println(Arrays.toString(callback.counts
		...
		System.out.println();
		System.out
				.println("Let's wait for the copy thread to finish cleaning up...");
		thread.join();
	...
	public List<String> fetchAutocompleteList(Jedis conn, String user,
			String prefix) {
	...
	public Set<String> autocompleteOnPrefix1(Jedis conn, String guild,
			String prefix){
...
//		conn.zadd(zsetName, 0, end);
		...
		for(;;){
			conn.watch(zsetName);
			Transaction trans = conn.multi();
			trans.zrangeByLex(zsetName, end, start);
	...
	public Set<String> autocompleteOnPrefix(Jedis conn, String guild,
			String prefix) {
		String[] range = findPrefixRange(prefix);
		String start = range[0];
		String end = range[1];
		...
		while (true) {
			...
			System.out.println(sindex + " " + eindex + " " + erange);
			Transaction trans = conn.multi();
	...
	public String acquireFairSemaphore(Jedis conn, String semname, int limit,
			long timeout) {
		String identifier = UUID.randomUUID().toString();
	...
	public boolean releaseFairSemaphore(Jedis conn, String semname,
			String identifier) {
		Transaction trans = conn.multi();
		trans.zrem(semname, identifier);
	...
	public String executeLater(Jedis conn, String queue, String name,
			List<String> args, long delay) {
		Gson gson = new Gson();
		String identifier = UUID.randomUUID().toString();
		String itemArgs = gson.toJson(args);
		String item = gson.toJson(new String[] { identifier, queue, name,
				itemArgs });
		if (delay > 0) {
	...
	public String createChat(Jedis conn, String sender, Set<String> recipients,
			String message) {
	...
	public String sendMessage(Jedis conn, String chatId, String sender,
			String message) {
	...
	public List<ChatMessages> fetchPendingMessages(Jedis conn, String recipient) {
		...
		for (Tuple tuple : seenList) {
			...
			trans.zrangeByScore("msgs:" + chatId, String.valueOf(seenId + 1),
					"inf");
		}
		List<Object> results = trans.exec();
		...
		while (seenIterator.hasNext()) {
			...
			seenUpdates
					.add(new Object[] { "seen:" + recipient, seenId, chatId });

			Set<Tuple> minIdSet = conn.zrangeWithScores("chat:" + chatId, 0, 0);
			if (minIdSet.size() > 0) {
				msgRemoves.add(new Object[] { "msgs:" + chatId,
						minIdSet.iterator().next().getScore() });
			}
		...
		while (true) {
			...
			for (ChatMessages messages : fdata) {
				for (Map<String, Object> message : messages.messages) {
					...
					InputStream in = new RedisInputStream(conn, messages.chatId
							+ logFile);
	...
	public class PollQueueThread extends Thread {
		...
		public void run() {
			while (!quit) {
				...
				if (item == null
						|| item.getScore() > System.currentTimeMillis()) {
					try {
						sleep(10);
					} catch (InterruptedException ie) {
						Thread.interrupted();
